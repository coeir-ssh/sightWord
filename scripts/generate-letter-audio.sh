...
for letter in a b c d e f g h i j k l m n o p q r s t u v w x y z; do
  ...
  espeak-ng -v en-us+f3 -s 140 -p 55 -a 180 "$upper" --stdout 2>/dev/null |
    ffmpeg -y -hide_banner -loglevel error -i - \
      -af "silenceremove=start_periods=1:start_silence=0.02:start_threshold=-40dB:stop_periods=1:stop_silence=0.1:stop_threshold=-40dB" \
